...
export interface ExecutionRecord<InputType = unknown, OutputType = unknown, B extends Boundaries = Boundaries> {
  /** Optional UUID v7 identifier for this execution record - will be generated by hive-sdk if not present */
  uuid?: string
...
> implements TaskInstanceType<Func, B> {
  ...
  isValid<T extends Record<string, unknown> = Parameters<Func>[0]>(argv?: T): boolean {
    ...
    // For empty schemas, normalize undefined input to empty object
    const normalizedInput = this._isEmptySchema() && argv === undefined ? {} as T : argv
    const result = this._schema.safeParse(normalizedInput)
  ...
  */
  ...
  async safeRun (argv?: Parameters<Func>[0]): Promise<[
    ...
    // For empty schemas, normalize undefined input to empty object
    const normalizedInput = this._isEmptySchema() && argv === undefined
      ? {} as Parameters<Func>[0]
